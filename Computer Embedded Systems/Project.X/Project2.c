...
void main(void) { //configure switches to input
    ...
    while (1) {
        ...
        {
            DADH(sin_data, cos_data);

            if (mode != 7) {
                playBeep();
                writeToLCD("Mode 7: Pizza       ", 0);
...
void DADH(int * data, int * data2) {
    int y = 0;
    int x;
    int counter;
    for (x = 0; x < COS_DATA_POINTS; x++) {//circle
        PORT(data[x]);
        PORT_B(data2[x]);
    }
    
    for (x = 0; x < COS_DATA_POINTS * 2; x++) {
        PORT(x);
        PORT_B(130);
    }
    x=25;
    y=35;
    for (counter = 0; counter < COS_DATA_POINTS*1.05; counter++) {
        PORT(x);
        PORT_B(y);
        x++;
        y++;
    ...
    x=230;
    y=45;
    for (counter = 0; counter < COS_DATA_POINTS; counter++) {
        PORT(x);
        PORT_B(y);
        x--;
        y++;
    ...
    for (y = 0; y < COS_DATA_POINTS * 2; y++) {
        PORT(130);
        PORT_B(y);
    }
}

void DASinCos(int * data, int * data2) {
